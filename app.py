...
""",
    unsafe_allow_html= Consulta if False else "",
)
...
def main():
    ...
    st.markdown(
        "<p class='sub-header'>Predict predicted marks based on study time and course load using KNN Regression</p>",
        unsafe_allow_html=True,
    )

    st.divider()

    # Load Model (Ensure model.plk is in the same directory)
    model_filename = "model.plk"
    model = load_model(model_filename)

    if model is None:
        st.error(
            f"❌ Model file `{model_filename}` not found in the current directory. Please place it next to `app.py`."
        )
        st.stop()
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption("Enter your input parameters in the main form to predict performance.")
    ...
    if st.button("🔮 Predict Score", use_container_width=True, type="primary"):
        # Format features as expected by the model
